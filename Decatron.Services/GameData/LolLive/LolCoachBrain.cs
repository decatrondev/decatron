    ...
    public class LolCoachBrain
    {
        ...
        private static string SystemPrompt(StreamerContext ctx, string kind, LivePhaseInfo phase)
        {
            ...
            var focus = kind switch
            {
                ...
                "briefing" => en ? "\nEvent: the streamer just opened the LoL client. Give a 2-3 sentence session briefing from the data (rank/LP, lastSession = the previous play session, best champion this week, streak, today's goal if any). If todaySoFar exists they already played this session and are coming back from a break: open with today's record, never greet as if the day were starting. Never say there is no data when some field has it. comment only; tips = up to 2 short focus points. Rest null." : "\nEvento: el streamer acaba de abrir el cliente de LoL. Da un briefing de 2-3 frases con los datos (rango/LP, lastSession = su sesión de juego anterior, mejor campeón de la semana, racha, objetivo de hoy si hay). Si hay todaySoFar, ya jugó en esta sesión y vuelve de una pausa: arranca con su récord de hoy, nunca saludes como si recién empezara el día. Nunca digas que no hay datos si algún campo los trae. Solo comment; tips = hasta 2 focos cortos. El resto null.",
                "lobby" => en ? "\nEvent: friends joined the lobby. Two or three sentences from lobbyMembers: the record with each of them (gamesTogether), their public rank and last-20 form (winRate, streak: positive = win streak, negative = loss streak), who is the strongest / who comes tilted, and a role suggestion from their top champions if obvious. Talk about them like a coach who scouted them, never mock their rank. comment only. Rest null." : "\nEvento: entraron amigos al lobby. Dos o tres frases con lobbyMembers: el récord con cada uno (gamesTogether), su rango público y su forma en las últimas 20 (winRate, streak: positivo = racha de victorias, negativo = de derrotas), quién es el más fuerte / quién viene tiltado, y sugerencia de roles por sus top campeones si es obvia. Habla de ellos como un coach que los scouteó, nunca te burles de su rango. Solo comment. El resto null.",
