...
export const DetectionTab: React.FC<Props> = ({ instance, detection, enabledGames, onChange, onRefresh }) => {
    const { t } = useTranslation('games', { keyPrefix: 'gameOverlays.detection' });
    const [busy, setBusy] = useState(false);
    const [msg, setMsg] = useState<string | null>(null);

    const force = async (game: string | null) => {
        setBusy(true); setMsg(null);
        try { await gameOverlaysApi.forceGame(game); await onRefresh(); setMsg(game ? t('forcedMsg', { game: GAME_NAMES[game as GameId] }) : t('autoMsg')); }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                    <div>
                        ...
                        <SelectInput value={instance.detectionMode} onChange={v => onChange(v === 'auto' ? { detectionMode: 'auto', forcedGame: null } : { detectionMode: 'manual' })}
                            options={[{ value: 'auto', label: t('modeAuto') }, { value: 'manual', label: t('modeManual') }]} />
                    </div>
                    {instance.detectionMode === 'manual' && (
                        <div>
                            <Label>{t('fixedGame')}</Label>
                    ...
                    <div>
                        ...
                        <SelectInput value={instance.idleBehavior} onChange={v => onChange({ idleBehavior: v as any })}
                            options={[{ value: 'hide', label: t('idleHide') }, { value: 'multi_card', label: t('idleMultiCard'), disabled: true }]} />
                    </div>
                </div>
            </Card>
