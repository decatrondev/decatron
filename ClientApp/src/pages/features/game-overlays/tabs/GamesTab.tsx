...
export const GamesTab: React.FC<Props> = ({ games, accounts, catalog, limits, onChange, onDesign }) => {
    const { t } = useTranslation('games', { keyPrefix: 'gameOverlays.games' });
    return (
                                            ...
                                            return (
                                                <div key={a.id} className={`flex items-center justify-between gap-2 ${disabled ? 'opacity-50' : ''}`}>
                                                    <Checkbox
                                                        ...
                                                        label={`${a.displayName} · ${a.fullName}${a.verified ? '' : ` (${t('unverified')})`}`}
                                                    />
                                    ...
                                    <div className="space-y-2 mt-1">
                                        <SelectInput
                                            ...
                                            options={[
                                                ...
                                                { value: 'interval', label: `${t('rotationInterval')}${canInterval ? '' : ' — Supporter'}`, disabled: !canInterval },
                                                { value: 'active_first', label: `${t('rotationActive')}${canActive ? '' : ' — Premium'}`, disabled: !canActive },
                                            ]}
                                        />
