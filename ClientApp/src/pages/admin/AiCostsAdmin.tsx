...
import { ArrowLeft, CircleDollarSign, Loader2, RefreshCw, Save, ExternalLink } from 'lucide-react';
...
interface Summary {
    ...
    recent: { id: number; module: string; provider: string; model: string; channelName: string | null; promptTokens: number; completionTokens: number; estimatedCostUsd: number; responseTimeMs: number; success: boolean; errorMessage: string | null; usedAt: string }[];
}
        ...
        <div className="max-w-[1400px] mx-auto space-y-6">
            ...
            {data && (
                <>
                    ...
                    <div className={cardClass}>
                        <h2 className={h2}>Últimas llamadas</h2>
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm">
                                <thead><tr><th className={th}>Cuándo</th><th className={th}>Módulo</th><th className={th}>Modelo</th><th className={th}>Canal</th><th className={th}>in/out</th><th className={th}>ms</th><th className={`${th} text-right`}>Costo</th></tr></thead>
                                <tbody className="text-[#1e293b] dark:text-[#f8fafc]">
                                    {data.recent.map(r => (
                                        <tr key={r.id} className={`${tr} ${r.success ? '' : 'text-red-500'}`} title={r.errorMessage ?? undefined}>
                                            ...
                                            <td className="text-xs">{r.channelName ?? '—'}</td>
                                            <td className="font-mono text-xs">{r.promptTokens}/{r.completionTokens}</td>
                                            <td className="text-xs">{r.responseTimeMs}</td>
                                            <td className="text-right font-mono text-xs">{r.success ? usd(r.estimatedCostUsd) : (r.errorMessage ?? 'error').slice(0, 30)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
