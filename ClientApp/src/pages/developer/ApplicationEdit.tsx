...
import {
    ...
    AlertTriangle, Code2, Shield, Globe
} from 'lucide-react';
...
export default function ApplicationEdit() {
    ...
    const loadApp = async () => {
        try {
            ...
            if (res.data.success) {
                ...
                setSelectedScopes(new Set(app.scopes));
            }
        } catch (err) {
            setNotFound(true);
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setSaving(true);
            const res = await api.put(`/developer/apps/${id}`, {
                name: name.trim(),
            ...
            if (res.data.success) {
                addToast('Aplicacion actualizada', 'success');
                navigate('/developer');
            }
        } catch (err: any) {
            const errorMsg = err.response?.data?.error || 'Error al actualizar aplicacion';
